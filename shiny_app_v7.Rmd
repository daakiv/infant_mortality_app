...
## for inteli section
data1 <- read.csv("/Users/damadi/Desktop/samp_data/infants2015.csv")
...
# Change Status for January 2015 to blanks ??
...
p<-ggplot(monthly_counts, aes(x=study_month, y=prop)) + geom_point() +
  ...
  xlab("Years") + ylab("Death rate per 1,000 live births") +
...
# output$articles2 <- renderValueBox({
#  coverage <-table(infants$immun)
#   valueBox(
#     "",
#     pie(coverage$Freq, labels = labs, col = c("red", "yellow", "blue"),
# main="Vaccine"),
#     icon="bar-chart-o",
#     color = "teal")
# })
# 
# valueBoxOutput("articles2")
...
#   aes(long, lat, group = group, fill = mean_subloc) + 
#   geom_polygon() + 
# scale_fill_gradient2(low = "blue",midpoint = 4, high = muted("darkblue"))
# +  geom_path(color="white") +
#   coord_equal() 
# 
# distcenters <- klf_shp %>% 
#   group_by(id) %>%
#   summarise(clat = median(lat), clong = median(long))
# 
#  p<- ggplot() +  geom_map(data = data1_sum, aes(map_id = sublocn, fill = mean_subloc ), 
#                     map = klf_shp) + expand_limits(x = klf_shp$long, y = klf_shp$lat) +
# scale_colour_brewer(palette ="RdBu") +
#  #with(distcenters, annotate(geom="text", x = clong, y=clat, label = id, size = 2.5)) +
#   geom_text(data = distcenters, aes(x = clong, y = clat, label = id, size = 0.001)) +
